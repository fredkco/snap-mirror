...
process_queue() {
  ...
  while true; do
    ...
    while IFS= read -r item || [[ -n "$item" ]]; do
      ...
      elif is_downloaded "$snap_name"; then
        snap_file="$(find_downloaded_snap_file "$snap_name")"
        if [[ -n "$snap_file" && -f "$snap_file" ]]; then
          if [[ "$selector_kind" == "channel" && -n "$selector_value" ]]; then
            log "Skipping already downloaded: $snap_name (channel $selector_value)"
          else
            log "Skipping already downloaded: $snap_name"
          fi
          enqueue_dependencies_from_local_snap "$snap_name" "$snap_file"
          clear_failed_for_snap "$snap_name"
          continue
          ...
          if [[ "$selector_kind" == "channel" && -n "$selector_value" ]]; then
            log "State says downloaded but file is missing, retrying: $snap_name (channel $selector_value)"
          else
            log "State says downloaded but file is missing, retrying: $snap_name"
          fi
        fi
      fi
...
generate_index_html() {
...

  {
    ...
    echo '  <p>Generated by snap-fetch.sh</p>'
	echo '  <p>'
